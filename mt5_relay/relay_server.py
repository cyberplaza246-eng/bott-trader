"""
MT5 Relay Server — runs on your Windows machine alongside MetaTrader5.

This exposes MT5 operations as a REST API so the trading bot running
on Linux/Codespace can place real orders on your demo account.

Setup (on Windows):
  1. pip install flask MetaTrader5
  2. Open MetaTrader5 and log in to your demo account
  3. python relay_server.py
  4. Expose with ngrok:  ngrok http 5555
  5. Copy the ngrok URL to your .env file as MT5_RELAY_URL

Endpoints:
  GET  /ping               — health check
  GET  /account             — account info
  GET  /balance             — current balance
  GET  /equity              — current equity
  GET  /candles?pair=...    — OHLCV data
  GET  /price?pair=...      — latest bid/ask
  GET  /positions           — open positions
  POST /order               — place an order
  POST /close               — close a position
"""
import sys
import json
from datetime import datetime
from flask import Flask, request, jsonify

try:
    import MetaTrader5 as mt5
except ImportError:
    print("ERROR: MetaTrader5 package not installed. Run: pip install MetaTrader5")
    sys.exit(1)

app = Flask(__name__)

# ── Auth token (set to match your .env MT5_RELAY_TOKEN) ──
RELAY_TOKEN = "change-me-to-a-secret"


def check_auth():
    """Simple bearer token check."""
    token = request.headers.get("Authorization", "").replace("Bearer ", "")
    if token != RELAY_TOKEN:
        return jsonify({"error": "Unauthorized"}), 401
    return None


@app.before_request
def auth_middleware():
    if request.endpoint == "ping":
        return  # Allow unauthenticated health checks
    err = check_auth()
    if err:
        return err


# ── Health ────────────────────────────────────────────────────────────

@app.route("/ping")
def ping():
    connected = mt5.terminal_info() is not None
    return jsonify({"status": "ok", "mt5_connected": connected})


# ── Account ───────────────────────────────────────────────────────────

@app.route("/account")
def account():
    info = mt5.account_info()
    if not info:
        return jsonify({"error": "Not connected to MT5"}), 500
    return jsonify({
        "login": info.login,
        "balance": info.balance,
        "equity": info.equity,
        "margin_free": info.margin_free,
        "margin": info.margin,
        "profit": info.profit,
        "leverage": info.leverage,
        "currency": info.currency,
        "server": info.server,
    })


@app.route("/balance")
def balance():
    info = mt5.account_info()
    if not info:
        return jsonify({"error": "Not connected"}), 500
    return jsonify({"balance": info.balance})


@app.route("/equity")
def equity():
    info = mt5.account_info()
    if not info:
        return jsonify({"error": "Not connected"}), 500
    return jsonify({"equity": info.equity})


# ── Market Data ───────────────────────────────────────────────────────

@app.route("/candles")
def candles():
    pair = request.args.get("pair", "EURUSD")
    tf_minutes = int(request.args.get("timeframe", 5))
    count = int(request.args.get("count", 100))

    tf_map = {
        1: mt5.TIMEFRAME_M1,
        5: mt5.TIMEFRAME_M5,
        15: mt5.TIMEFRAME_M15,
        60: mt5.TIMEFRAME_H1,
        240: mt5.TIMEFRAME_H4,
        1440: mt5.TIMEFRAME_D1,
    }
    tf = tf_map.get(tf_minutes)
    if not tf:
        return jsonify({"error": f"Unsupported timeframe: {tf_minutes}"}), 400

    # Try symbol formats: EUR/USD → EURUSD
    for symbol in [pair, pair.replace("/", "")]:
        if mt5.symbol_select(symbol, True):
            rates = mt5.copy_rates_from_pos(symbol, tf, 0, count)
            if rates is not None and len(rates) > 0:
                data = []
                for r in rates:
                    data.append({
                        "datetime": datetime.utcfromtimestamp(r[0]).isoformat(),
                        "open": float(r[1]),
                        "high": float(r[2]),
                        "low": float(r[3]),
                        "close": float(r[4]),
                        "volume": int(r[5]),       # tick_volume
                        "real_volume": int(r[6]),
                    })
                return jsonify({"candles": data})

    return jsonify({"error": f"No data for {pair}", "last_error": str(mt5.last_error())}), 404


@app.route("/price")
def price():
    pair = request.args.get("pair", "EURUSD")
    for symbol in [pair, pair.replace("/", "")]:
        tick = mt5.symbol_info_tick(symbol)
        if tick:
            return jsonify({
                "bid": tick.bid,
                "ask": tick.ask,
                "last": tick.last,
                "time": datetime.utcfromtimestamp(tick.time).isoformat(),
            })
    return jsonify({"error": f"No price for {pair}"}), 404


# ── Positions ─────────────────────────────────────────────────────────

@app.route("/positions")
def positions():
    pair = request.args.get("pair")
    if pair:
        # Try both formats
        pos = mt5.positions_get(symbol=pair) or mt5.positions_get(symbol=pair.replace("/", ""))
    else:
        pos = mt5.positions_get()

    if not pos:
        return jsonify({"positions": []})

    return jsonify({
        "positions": [
            {
                "ticket": p.ticket,
                "pair": p.symbol,
                "type": "BUY" if p.type == 0 else "SELL",
                "volume": p.volume,
                "open_price": p.price_open,
                "current_price": p.price_current,
                "profit": p.profit,
                "sl": p.sl,
                "tp": p.tp,
                "open_time": datetime.utcfromtimestamp(p.time).isoformat(),
            }
            for p in pos
        ]
    })


# ── Orders ────────────────────────────────────────────────────────────

@app.route("/order", methods=["POST"])
def place_order():
    data = request.json
    pair = data.get("pair", "EURUSD")
    order_type = data.get("type", "BUY")
    lot_size = float(data.get("lot_size", 0.01))
    sl = float(data.get("stop_loss", 0))
    tp = float(data.get("take_profit", 0))

    # Resolve symbol
    symbol = pair.replace("/", "")
    if not mt5.symbol_select(symbol, True):
        symbol = pair
        if not mt5.symbol_select(symbol, True):
            return jsonify({"error": f"Symbol {pair} not found"}), 404

    # Get current price for market order
    tick = mt5.symbol_info_tick(symbol)
    if not tick:
        return jsonify({"error": "Cannot get current price"}), 500

    action_type = mt5.ORDER_TYPE_BUY if order_type == "BUY" else mt5.ORDER_TYPE_SELL
    price = tick.ask if order_type == "BUY" else tick.bid

    req = {
        "action": mt5.TRADE_ACTION_DEAL,
        "symbol": symbol,
        "volume": lot_size,
        "type": action_type,
        "price": price,
        "sl": sl,
        "tp": tp,
        "deviation": 20,
        "magic": 234000,
        "comment": "AI Trading Bot",
        "type_time": mt5.ORDER_TIME_GTC,
        "type_filling": mt5.ORDER_FILLING_IOC,
    }

    result = mt5.order_send(req)
    if result is None:
        return jsonify({"error": "order_send returned None", "last_error": str(mt5.last_error())}), 500

    if result.retcode != mt5.TRADE_RETCODE_DONE:
        return jsonify({
            "error": f"Order failed: {result.comment}",
            "retcode": result.retcode,
        }), 400

    return jsonify({
        "ticket": result.order,
        "pair": symbol,
        "type": order_type,
        "volume": lot_size,
        "price": price,
    })


@app.route("/close", methods=["POST"])
def close_position():
    data = request.json
    pair = data.get("pair", "EURUSD")
    volume = float(data.get("volume", 0))

    symbol = pair.replace("/", "")
    positions = mt5.positions_get(symbol=symbol)
    if not positions:
        positions = mt5.positions_get(symbol=pair)
    if not positions:
        return jsonify({"error": f"No open position for {pair}"}), 404

    pos = positions[0]
    close_type = mt5.ORDER_TYPE_SELL if pos.type == 0 else mt5.ORDER_TYPE_BUY
    tick = mt5.symbol_info_tick(pos.symbol)
    price = tick.bid if pos.type == 0 else tick.ask

    req = {
        "action": mt5.TRADE_ACTION_DEAL,
        "symbol": pos.symbol,
        "volume": volume if volume > 0 else pos.volume,
        "type": close_type,
        "price": price,
        "deviation": 20,
        "magic": 234000,
        "comment": "AI Bot Close",
        "type_time": mt5.ORDER_TIME_GTC,
        "type_filling": mt5.ORDER_FILLING_IOC,
    }

    result = mt5.order_send(req)
    if result.retcode != mt5.TRADE_RETCODE_DONE:
        return jsonify({"error": f"Close failed: {result.comment}", "retcode": result.retcode}), 400

    return jsonify({"ticket": result.order, "closed": pos.ticket})


# ── Main ──────────────────────────────────────────────────────────────

if __name__ == "__main__":
    print("Initializing MT5...")
    if not mt5.initialize():
        print(f"MT5 init failed: {mt5.last_error()}")
        sys.exit(1)

    info = mt5.account_info()
    print(f"✅ Connected to MT5 | Account: {info.login} | Balance: {info.balance}")
    print(f"🌐 Relay server starting on http://0.0.0.0:5555")
    print(f"   Expose with: ngrok http 5555")
    print(f"   Then set MT5_RELAY_URL in your .env")

    app.run(host="0.0.0.0", port=5555, debug=False)
